...
serve(async (req) => {
  try {
    ...
    console.log(`Processing weekly streaks for week ending ${lastSunday.toISOString()}`);
    console.log(`Current ISO week: ${currentISOWeek}`);

    // Get all employees with their targets and current streak info
    const { data: employees, error: employeesError } = await supabase.rpc('get_employees_for_streak_check');
    ...
    for (const employee of employees) {
      try {
        ...
        if (employee.last_target_met_week === currentISOWeek) {
          console.log(`Skipping ${employee.full_name} - already processed for week ${currentISOWeek}`);
          continue;
        }

        // Calculate total learning hours for last week
        const { data: weekSummary, error: summaryError } = await supabase.rpc('get_user_time_summary', {
        ...
        if (summaryError) {
          throw new Error(`Failed to get time summary for ${employee.full_name}: ${summaryError.message}`);
        }
        ...
        console.log(`${employee.full_name}: ${weeklyHours.toFixed(2)}h / ${targetHours}h - ${targetMet ? 'TARGET MET' : 'target not met'}`);

        if (targetMet) {
          // Increment streak
          const newStreak = (employee.current_streak || 0) + 1;
          const newLongestStreak = Math.max(newStreak, employee.longest_streak || 0);
          ...
          const badgeAwarded = await awardStreakBadge(supabase, employee.user_id, newStreak);
...
// Award streak badge based on milestone
async function awardStreakBadge(supabase: any, userId: string, streakWeeks: number): Promise<string | null> {
  let badgeSlug = null;
  ...
  if (badgeSlug) {
    try {
      ...
      console.log(`Awarded ${badgeSlug} badge to user ${userId} for ${streakWeeks} week streak`);
      return badgeSlug;
    } catch (error) {
...
// Log cron execution
async function logCronExecution(supabase: any, jobName: string, results: any[]) {
  const successCount = results.filter(r => r.status === 'success' || r.status === 'streak_reset').length;
