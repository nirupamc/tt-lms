...
function checkStringMatch(code: string, expectedOutput: string): CheckResult {
  ...
  return {
    passed,
    output: passed ? 'Output matches expected result!' : 'Output does not match expected result.',
...
function validateSyntax(code: string, language: string): { valid: boolean; error?: string } {
  ...
  switch (language) {
    ...
    case 'sql':
      ...
      const hasSqlKeyword = sqlKeywords.some(kw => trimmed.toLowerCase().includes(kw));
      if (!hasSqlKeyword) {
...
serve(async (req) => {
  ...
  try {
    ...
    const { code, language, module_id, user_id, expected_output, test_cases } = body;
